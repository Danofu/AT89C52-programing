...
{
  if (row == 0) Lcd_WriteControl (0x80 + column);
  if (row == 1) Lcd_WriteControl (0xc0 + column);
  if (row == 2) Lcd_WriteControl (0x90 + column);
  if (row == 3) Lcd_WriteControl (0xd0 + column);
}
...
void Lcd_DisplayScreen (char *ptr)
{
  ...
  Lcd_DisplayRow(2, ptr + 32);
  Lcd_DisplayRow(3, ptr + 48);
}
...
void AddCustomCharacter(unsigned char Character[], unsigned char CGRAMAddress)
{
  unsigned char i;
  Lcd_WriteControl(CGRAMAddress);
  for (i = 0; i < 8; i++) { Lcd_WriteData(Character[i]); }
}
